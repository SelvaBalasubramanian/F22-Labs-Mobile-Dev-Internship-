...
/*
    ...
    All words in DB file are stored in Trie Datastructure.
...
int main(){
    str = "elephantafraidofphotograph"; // Input String // modify here
    tnode *root ;
    root = new tnode();
    ...
    if(file.is_open()){
        ...
        while(getline(file,l,',')){
            ...
            remove(l.begin(),l.end(),'\n');
            cout<<l<<l.length()<<endl; //print each string to check certain diffculty in trimming string
    ...
    if(seprateString(root,root,0)) cout<<"Output : "<<str<<endl;
    else cout<<"Output : Invalid Input"<<endl;
...
/*
    FUNCTION : seprateString()
        Recursively traverse down till string matches in trie. If end of string occurs, traverse from 
    root again for the next word.
